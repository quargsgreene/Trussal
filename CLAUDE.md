...
- **Every program buffer self-declares its kind.** A performer's editor opens with `'personal program'`, a bot's code with `'bot program'`, the JPattern editor with `'metaprogram'` (`src/program-directive.js`). The directive is required with no fallback — an unlabeled buffer is rejected, not guessed. `mini` and `mondo` notation are both available in both editors, chosen per fragment by delimiter: `"…"` vs `` mondo`…` `` in the personal editor, `<…>`/`[…]` vs `(…)` s-expressions in JPattern.
