...
| `breakout-core.js` | Pure logic for `# breakout`/`# assign` (the Metaprogram's breakout-room directives — see `jpattern.md` and `MetaprogrammerParser.js`): the `{name, participants}` JSON literal shape and `resolveBreakoutState`, which folds a program's declared rooms and assignments (in source order — a later line wins) into one `{rooms, assignedTo}` view. `audio-net/Breakout.js` reads it to drive Jitsi's real breakout-rooms feature; see that module's doc comment for the "each room gets its own metaprogram" follow-up this does NOT yet implement |
