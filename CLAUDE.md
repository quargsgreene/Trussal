...
| `data-ref.js` | The one rule for what a data-pack reference means, shared by all four contexts that can hold one (Strudel and Text Cycles via the string-literal rewrite, Hydra via `H()`, Net Cycles via the metaprogram parser), for the same reason `hydra-code.js` exists. `"Weather:3"` is the third **column** (CSV/TSV) or top-level property (JSON) — 1-based, unlike an audio bank's 0-based index, and never a sound. Packs are keyed by owning peerId and looked up in sorted peerId order, and `shuffle()` draws from the room's shared seeded PRNG, so a reference resolves identically in every browser and in the aggregator |
