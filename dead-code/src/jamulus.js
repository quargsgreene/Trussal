// ARCHIVED 2026-08-28 — moved out of src/jamulus.js as dead code.
//
// Two abandoned features:
//   1. Jamulus welcome-page panel + in-meeting banner. The entry point
//      `renderJamulusWelcomePanelAndBanner()` was imported by src/index.js but
//      its call site (index.js ~line 93) was commented out; the banner poll was
//      commented out inside `renderJamulusWelcomePanelAndBanner` itself.
//   2. In-page Jamulus relay client (`connectJamulusRelay` &c). Imported by
//      src/studio.js, but the only caller — an `onRelayClick()` handler — was
//      commented out there (also archived, see dead-code/src/studio.js).
//
// `getRoomNameFromUrl` and `JAMULUS_ROOM_MAP` stayed in src/jamulus.js; they
// are still used across the bundle.
//
// To revive, these imports are needed at the top of src/jamulus.js again:
...
// and `JAMULUS_ROOM_MAP` / `getRoomNameFromUrl` are imported from './jamulus.js'.

import { JAMULUS_ROOM_MAP, getRoomNameFromUrl } from '../../src/jamulus.js';
import { bootAudioEngine, attachNodeToChain, detachNodeFromChain, setJamulusMode } from '../../src/latency-instrument.js';
import { getLocalPeer } from '../../src/peer-state.js';

// ---- Welcome-page panel + banner -----------------------------------------
