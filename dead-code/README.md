...
- **WCRTT** (2026-09-01) — `src/audio-net/network-modulation/IncreaseRTT.js`
  ...
  removed at the time and was not touched. Room.js keeps the decay-driven
  cascaded lowpass from the 2026-08-29 removal rather than reverting to the old
  `wcrtt`-driven cutoff — `wcrtt` is just one more valid `METRIC_PER_SECOND`
  decay driver now, same as `wcl`/`wcpl`.
- **WCJ** (2026-09-20) — `src/audio-net/network-modulation/IncreaseJitter.js`
  brought back to `src/`; `wcj` re-added to `TIMING_METRICS`/`EFFECT_METRICS`/
  `CRUSH_METRICS`/`ECHO_METRICS` (parser + av-effects), `INDUCTIONS`, and the
  CRDT `modulation` channel; `computeWorstCaseMetrics` returns `wcj` again
  (worst measured audio RTP inter-arrival jitter, media-path RTCStats
  preferred over WS ping/pong — `peerJitter()` restored in
  `WorstCaseCalculationUtils.js`); the studio Network Metrics panel shows it.
  `RoomHealth.avDecouplingSeconds` reverts to its original `wcj`-driven stretch
  (the 2026-08-29 removal's repoint to `wcl` was a mechanical substitution, not
  a considered redesign, unlike Room.js's cascaded lowpass — see the WCRTT
  entry above — so it was reverted rather than kept). `Room.js`'s
  decay-driven cascaded lowpass from the 2026-08-29 removal was kept as-is;
  `wcj` is simply one more valid `METRIC_PER_SECOND` decay driver, same as
  `wcl`/`wcrtt`/`wcpl`.
