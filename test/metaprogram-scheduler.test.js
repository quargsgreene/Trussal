...
test('? drops the turn about half the time, per occurrence and identically everywhere', () => {
  ...
  // Both outcomes actually occur, near enough evenly.
  let kept = 0;
  for (let c = 1; c < 401; c += 2) kept += expandCycle(ast.participants, c).length;
...
test("a stretched turn's ? is decided once, not re-flipped at every cycle boundary", () => {
  ...
  for (let turn = 0; turn < 6; turn++) {
    const played = [0, 1, 2, 3].map(k => expandCycle(ast.participants, turn * 16 + 12 + k).length);
    assert.ok(
