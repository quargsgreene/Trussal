...
export function transpiler(input, options = {}) {
  ...
  // Pre-process *name: code → new StrudelButton("name: code") before Acorn parsing.
  // Tracks position deltas so mini-notation locations can be adjusted back to original positions.
  const buttonAdjustments = [];
  let processedOffset = 0;
  input = input.replace(/^\*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*:\s*(.+)$/mg, (match, name, expr, originalPos) => {
    // Use single quotes so the mini-notation plugin (which targets double-quoted strings) skips this argument.
    const escaped = expr.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
    const replacement = `new StrudelButton('${name}: ${escaped}')`;
    const delta = replacement.length - match.length;
    buttonAdjustments.push({ afterPos: originalPos + processedOffset + replacement.length, delta });
    processedOffset += delta;
    return replacement;
  });
  ...
  // Adjust mini-notation locations from processed-code positions back to original source positions.
  if (buttonAdjustments.length > 0 && context.miniLocations) {
    context.miniLocations = context.miniLocations.map(([from, to]) => {
      let shift = 0;
      for (const { afterPos, delta } of buttonAdjustments) {
        if (from >= afterPos) shift += delta;
      }
