...
// wcl/wcj/wcrtt, PERCENT for wcpl (`20` means 20 % loss, while metrics.wcpl
...
// cycle position by NumberPattern.js. Since that sampling and the metrics are
// identical on every client, so is the resulting audio.
//
// Pure math (echoParams) is separated from node construction (createEchoNode)
// for node:test.

import { sampleNumberPattern, isNumberPattern } from '../NumberPattern.js';
...
export const ECHO_METRICS = ['wcl', 'wcj', 'wcrtt', 'wcpl'];
...
export function echoIsPatterned(user) {
  ...
  return slots.some(s => isNumberPattern(s.scale) || isNumberPattern(s.bound));
}
...
export function echoParams(metrics, user, { cycleSeconds = 1, cyclePos = 0 } = {}) {
  ...
  for (const slot of slots) {
    const scale = sampleNumberPattern(slot.scale, cyclePos);
    const bound = sampleNumberPattern(slot.bound, cyclePos);
